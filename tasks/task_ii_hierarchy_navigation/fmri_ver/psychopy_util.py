...
class Presenter:
    ...
    def show_instructions(self, instructions, position=(0, 0), other_stim=(), key_to_continue='space',
        ...
        for i, instr in enumerate(instructions):
            instr_stim = visual.TextStim(self.window, text=instr, pos=position, wrapWidth=1.7)
            log_text = 'Instruction: ' + instr[:30].replace('\n', ' ')
            self.logger.info(log_text + '...' if len(instr) >= 30 else log_text)
            if type(key_to_continue) != list:
                key_to_continue = [key_to_continue]
    ...
    def select_from_stimuli(self, stimuli, values, response_keys, max_wait=float('inf'), post_selection_time=1,
        ...
        A feedback stimulus can be optionally displayed, either in its own position or next to the selected stimulus.
        ...
        if highlight is None:
            original_color = selected_stim.fillColor
            selected_stim.fillColor = '#A0A0A0'
            self.draw_stimuli_for_duration(stimuli, post_selection_time, post_select_wait_trigger)
            selected_stim.fillColor = original_color
        else:
        ...
        if correctness_func is not None:
            ...
            if feedback_stims is not None and len(feedback_stims) == 2:
                ...
                stimuli.insert(0, stims[0])
                stimuli.append(stims[1])
