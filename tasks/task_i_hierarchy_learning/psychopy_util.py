...
class Presenter:
    ...
    def show_instructions(self, instructions, position=(0, 0), other_stim=(), next_key='space',
                          next_page_text='Press {key} to continue', next_page_pos=(0.0, -0.8), duration=None,
                          wait_trigger=False):
        """
        ...
        :param next_key: (string) the key to continue to the next page
        ...
        if next_page_text is not None:
            text = next_page_text.format(key=(next_key.upper() if len(next_key) == 1 else next_key))
